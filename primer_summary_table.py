...
def makeset_probes(x):
    ...
    if 'ProbeID' in x:
        ...
        #sort is required because set operation is not deterministic in ordering
        return tuple(sorted(set([fp,rp,probe])))
    else:
        return tuple(sorted(set([fp,rp])))
...
parser.add_argument('-i', required=False, default=None, type=str, help='optional inclusivity .tsv coverage table')
...
if myargs.e:
    exclusive = []
    with open(myargs.e,'r') as infile:
        exclusive_dict = json.load(infile)
    for target in exclusive_dict.keys():
        exclusive.extend(list(map(tuple,[sorted(e) for e in exclusive_dict[target]])))
    assay_mask = out_df.apply(makeset_probes,axis=1)
    out_df = out_df[assay_mask.isin(exclusive)]
...
if myargs.i:
    inclusive_df = pd.read_table(myargs.i,index_col=0)
    inclusive_df = inclusive_df.pivot_table(index="primer_group",columns = 'label',values='Coverage')
    inclusive_df.columns = [str(c)+'_coverage' for c in inclusive_df.columns]
    out_df = out_df.merge(inclusive_df, left_index=True, right_index=True, how='inner')
