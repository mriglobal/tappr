...
parser.add_argument('-n',default=0, type=int, help='Number of samples for resampling.')
...
if myargs.n:
    ...
    for i in recdata:
        ...
        seq_groups = pd.DataFrame({'seq_id':[seq_id for seq_id in rec_dict[i]],'groups':group_labels}).groupby('groups')
        stratified = [np.random.choice(seq_groups.get_group(np.random.choice(groups))['seq_id']) for _ in range(myargs.n)]
